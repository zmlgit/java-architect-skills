...
- **Bean Scopes**: Finds misuse of Request/Session scopes in singleton contexts
- **Circular Dependencies**: Identifies potential cycles in `@Autowired` constructors

---

## 🚀 Installation

### Method 1: Automatic (via npx)
You can run the server directly using `npx` (requires Node.js and Python 3.10+):

1.  **Configure Claude Desktop**:
    Edit your `claude_desktop_config.json` to use `npx`:
    ```json
    {
      "mcpServers": {
        "spring-reviewer": {
          "command": "npx",
          "args": ["-y", "spring-reviewer"]
        }
      }
    }
    ```
...
3.  Configure Claude Desktop:
    ```json
    {
      "mcpServers": {
        "spring-reviewer": {
          "command": "python3",
          "args": ["/absolute/path/to/spring-reviewer/src/server.py"]
        }
      }
    }
    ```
